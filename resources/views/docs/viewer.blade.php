...
<head>
    ...
    <style>
        ...
        .content-area h1 { font-size: 4rem; font-weight: 900; margin-bottom: 2rem; text-transform: uppercase; letter-spacing: -0.05em; line-height: 1; }
        ...
        /* Sidebar Styling */
        .sidebar-link { display: block; width: 100%; padding: 1.25rem 2rem; border-bottom: 4px solid #000; font-size: 1.25rem; font-weight: 900; text-transform: uppercase; text-decoration: none; color: #000; background: #fff; position: relative; transition: all 0.2s; }
        .sidebar-link:hover { background: #3b82f6; color: #fff; padding-left: 2.5rem; }
        .sidebar-link.active { background: #000; color: #fff; padding-left: 2.5rem; }
        .sidebar-link.active::after { content: '→'; position: absolute; right: 2rem; }
...
<body class="min-h-screen flex flex-col md:flex-row">
    ...
    <!-- Sidebar -->
    <aside id="sidebar" class="hidden md:block w-full md:w-96 bg-white border-r-8 border-black shrink-0 sticky top-0 h-screen overflow-y-auto z-40">
        <div class="p-12 border-b-8 border-black flex justify-center bg-blue-500">
            <a href="/docs">
                <div class="bg-white p-6 border-4 border-black shadow-[12px_12px_0px_0px_rgba(0,0,0,1)]">
                    <img src="/docs/logo.svg" alt="Velo Logo" class="h-16 w-auto">
                </div>
            </a>
        </div>
        
        <!-- Search Box -->
        <div class="p-8 border-b-8 border-black bg-white">
            <form action="/docs/search" method="GET" class="relative">
                <input type="text" name="q" value="{{ $search_query ?? '' }}" placeholder="SEARCH DOCS..." 
                    class="w-full px-6 py-4 border-4 border-black font-black uppercase tracking-tighter placeholder-black/50 focus:bg-blue-500 focus:text-white transition-all outline-none">
                <button type="submit" class="absolute right-4 top-1/2 -translate-y-1/2 font-black">↵</button>
            </form>
        </div>
        ...
        <nav class="flex flex-col">
            @foreach($files as $fileName)
                @if($fileName !== 'index.md')
                    @php $isActive = request()->is('docs/' . $fileName); @endphp
                    <a href="/docs/{{ $fileName }}" class="sidebar-link {{ $isActive ? 'active' : '' }}">
                        {{ strtoupper(str_replace(['-', '.md'], [' ', ''], $fileName)) }}
                    </a>
                @endif
            @endforeach
        </nav>
        ...
        <div class="max-w-4xl mx-auto">
                ...
                @if(isset($search_results))
                    ...
                    <div class="space-y-8">
                        @foreach($search_results as $result)
                            <div class="bg-white p-10 border-8 border-black shadow-[16px_16px_0px_0px_rgba(0,0,0,1)] -rotate-1 hover:rotate-0 transition-transform cursor-pointer" onclick="window.location='/docs/{{ $result['file'] }}'">
                                ...
                                <a href="/docs/{{ $result['file'] }}" class="text-blue-600 font-black uppercase tracking-widest border-b-4 border-blue-600 hover:bg-blue-600 hover:text-white transition-all">View Document →</a>
                            </div>
                        @endforeach
                    </div>
            ...
            <footer class="mt-40 pt-20 border-t-8 border-black flex flex-col md:flex-row justify-between items-start md:items-center gap-12">
                <div>
                    ...
                    <p class="font-black uppercase tracking-tighter">Open Source BaaS by Laravel</p>
                </div>
