...
const datetimeTypes = new Set(["timestamp", "datetime"]);
...
<template>
                ...
                <template v-if="!loading">
                    <TableRow v-for="record in records" :key="record.id"
                        ...
                        <TableCell v-for="column in columns" :key="`${record.id}-${column}`"
                            ...
                            <div v-if="isDatetimeColumn(column, columnTypes)" class="leading-tight whitespace-nowrap">
                            ...
                            <div v-else-if="isRelationColumn(column, relationFields)" class="flex flex-col gap-1">
                                <a v-for="relationId in normalizeRelationIds(record[column])"
                                    ...
                                    <template
                                        v-if="findNameField(relationFields[column]?.targetCollectionFields) && record.expand?.[column]">
                                        {{ getRelationLabel(record, column, relationId, relationFields) }}
                                    </template>
                                    <template v-else>
                                        <component :is="resolveCollectionFieldTypeIcon('relation')"
                                            class="h-3 w-3 text-muted-foreground" />
                                        <span>{{ relationFields[column]?.targetCollectionName || 'Record' }} ({{
                                            relationId }})</span>
                                    </template>
                                </a>
                                <span v-if="normalizeRelationIds(record[column]).length === 0">-</span>
                            </div>
                            <div v-else-if="isFileColumn(column, columnTypes)"
                                class="flex items-center gap-1.5 min-w-0">
                                <template v-for="(file, index) in displayFilesForDataTable(record[column])"
                                    :key="file.path">
                                    <div v-if="isImageFile(file)" class="relative group shrink-0">
                                        <a href="javascript:void(0)" class="shrink-0"
                                            ...
                                            <img v-if="resolveFileSourceUrl(file)?.protected"
                                            ...
                                            <img v-else-if="filePreviewUrl(file)" :src="filePreviewUrl(file)"
                                                :alt="filePreviewAlt(file)"
                                                class="h-10 w-10 rounded-md border bg-muted object-cover hover:ring-2 hover:ring-primary/50 transition-all"
                                                loading="lazy" />
                                        </a>
                                ...
                                <span v-if="fileCount(record[column]) > 3"
                                    class="text-[10px] font-bold text-muted-foreground whitespace-nowrap bg-muted px-1.5 py-0.5 rounded-sm">
                ...
                <PaginationContent v-slot="{ items }">
                    ...
                    <template v-for="(item, index) in items">
                        <PaginationItem v-if="item.type === 'page'" :key="index" :value="item.value" as-child>
                            <Button class="w-8 h-8 sm:w-9 sm:h-9 p-0"
    ...
    <Dialog :open="!!activePreview" @update:open="(val) => !val && (activePreview = null)">
        <DialogContent
            class="max-w-[90vw] max-h-[90vh] p-0 overflow-hidden bg-transparent border-none shadow-none flex items-center justify-center">
            <template v-if="activePreview">
                <DialogTitle class="sr-only">Image Preview</DialogTitle>
                <DialogDescription class="sr-only">Viewing {{ activePreview.name ?? activePreview.path }}
                </DialogDescription>
                <div class="relative flex items-center justify-center w-full h-full p-12">
                    ...
                    <img :src="objectUrls.get(resolveFileSourceUrl(activePreview)?.url) || filePreviewUrl(activePreview)"
                    ...
                    <div
                        class="absolute bottom-6 left-0 right-0 text-center animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <div
                            class="inline-block bg-black/60 px-4 py-2 rounded-full backdrop-blur-md ring-1 ring-white/10 shadow-xl">
                            <p class="text-white text-sm font-semibold tracking-wide">{{ activePreview.name ??
                                activePreview.path }}</p>
                        </div>
                    </div>
                </div>
            </template>
