...
const saveSettings = async () => {
  ...
  try {
    ...
    if (error.response?.status === 422) {
      ...
      toast.error("Please correctly fill in required fields.");
    } else {
      ...
      <CardContent class="space-y-4">
        ...
        <div class="grid gap-3 md:grid-cols-2">
          <div class="space-y-2">
            ...
            <select v-model="form.general.timezone" class="flex h-10 w-full items-center rounded-md border border-input bg-background px-3 py-2 text-sm">
          ...
          <div class="space-y-2">
            <Label>Locale</Label>
            <select v-model="form.general.locale" class="flex h-10 w-full items-center rounded-md border border-input bg-background px-3 py-2 text-sm">
        ...
        <div class="space-y-2">
          ...
          <p v-if="errors['general.contact_email']" class="text-xs text-red-500">{{ errors['general.contact_email'][0] }}</p>
        </div>

        <label class="flex items-center gap-2 text-sm cursor-pointer py-2">
          <Checkbox :checked="form.general.lock_schema_change" @update:checked="form.general.lock_schema_change = !form.general.lock_schema_change" />
    ...
    <Card>
      ...
      <CardContent class="space-y-4">
        <div class="space-y-2">
          ...
          <select v-model="form.storage.storage_driver" class="flex h-10 w-full items-center rounded-md border border-input bg-background px-3 py-2 text-sm">
            <option value="local">Local Disk</option>
            <option value="s3">AWS S3</option>
          </select>
          <p v-if="errors['storage.storage_driver']" class="text-xs text-red-500">{{ errors['storage.storage_driver'][0] }}</p>
        </div>
        ...
        <div v-if="form.storage.storage_driver === 's3'" class="space-y-4 pt-4 border-t">
          <div class="grid gap-3 md:grid-cols-2">
            ...
            <div class="space-y-2">
              ...
              <p v-if="errors['storage.s3_secret']" class="text-xs text-red-500">{{ errors['storage.s3_secret'][0] }}</p>
            </div>
          </div>
          <div class="grid gap-3 md:grid-cols-2">
            <div class="space-y-2">
              ...
              <p v-if="errors['storage.s3_region']" class="text-xs text-red-500">{{ errors['storage.s3_region'][0] }}</p>
            </div>
            <div class="space-y-2">
              <Label>S3 Bucket</Label>
              <Input v-model="form.storage.s3_bucket" placeholder="my-bucket" />
              <p v-if="errors['storage.s3_bucket']" class="text-xs text-red-500">{{ errors['storage.s3_bucket'][0] }}</p>
            </div>
          </div>
          <div class="space-y-2">
            <Label>S3 Endpoint (Optional)</Label>
            <Input v-model="form.storage.s3_endpoint" placeholder="https://s3.amazonaws.com" />
            <p v-if="errors['storage.s3_endpoint']" class="text-xs text-red-500">{{ errors['storage.s3_endpoint'][0] }}</p>
          </div>
        </div>
      </CardContent>
    </Card>
      ...
      <CardContent class="space-y-4">
        <div class="grid gap-3 md:grid-cols-2">
          <div class="space-y-2">
            ...
            <select v-model="form.email.mail_driver" class="flex h-10 w-full items-center rounded-md border border-input bg-background px-3 py-2 text-sm">
          ...
          <div class="space-y-2">
            ...
            <select v-model="form.email.mail_encryption" class="flex h-10 w-full items-center rounded-md border border-input bg-background px-3 py-2 text-sm">
              <option value="tls">TLS</option>
              <option value="ssl">SSL</option>
            </select>
            <p v-if="errors['email.mail_encryption']" class="text-xs text-red-500">{{ errors['email.mail_encryption'][0] }}</p>
          </div>
        </div>
        ...
        <div class="grid gap-3 md:grid-cols-2">
          <div class="space-y-2">
            ...
            <p v-if="errors['email.mail_username']" class="text-xs text-red-500">{{ errors['email.mail_username'][0] }}</p>
          </div>
          <div class="space-y-2">
            <Label>Password</Label>
            <Input v-model="form.email.mail_password" type="password" placeholder="••••••••" />
            <p v-if="errors['email.mail_password']" class="text-xs text-red-500">{{ errors['email.mail_password'][0] }}</p>
          </div>
        </div>
        ...
        <div class="grid gap-3 md:grid-cols-2">
          <div class="space-y-2">
            ...
            <p v-if="errors['email.mail_from_name']" class="text-xs text-red-500">{{ errors['email.mail_from_name'][0] }}</p>
          </div>
          <div class="space-y-2">
            <Label>From Address</Label>
            <Input v-model="form.email.mail_from_address" type="email" placeholder="noreply@example.com" />
            <p v-if="errors['email.mail_from_address']" class="text-xs text-red-500">{{ errors['email.mail_from_address'][0] }}</p>
          </div>
        </div>
      </CardContent>
    </Card>
